...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==='search_one'){var sq=body.query||'lower middle market manufacturer acquired 2025',sp=body.period||'2025',p2='Find real M&A deals matching: '+sq+'. Search prnewswire.com businesswire.com globenewswire.com acg.org middlemarketgrowth.org. Industrial manufacturers $10M-$200M revenue acquired by PE or strategic buyers. Return JSON array with: id(buyer-target-YYYYMM),date_announced,buyer,buyer_type(PE or Strategic),pe_firm,seller,target,market_segment(Advanced Manufacturing/Engineered Materials/Precision Components/ICT Data Center/Industrial Services/Other Industrial),deal_type(Platform/Add-on/Recapitalization/Sale),revenue_disclosed,banking_advisors(array),legal_buyer(array),legal_seller(array),description,platform_context,source,catskill_relevance(high/medium/low). Only real confirmed deals. Return [] if none.';try{var b2={model:'claude-sonnet-4-20250514',max_tokens:4000,messages:[{role:'user',content:p2}],tools:[{type:'web_search_20250305',name:'web_search'}]};var s2=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01'},body:JSON.stringify(b2),signal:AbortSignal.timeout(55000)});if(!s2.ok)return res.status(200).json({success:false,deals:[],error:'Claude '+s2.status});var d2=await s2.json();var t2=(d2.content||[]).filter(function(b){return b.type==='text';}).map(function(b){return b.text;}).join('');var m2=t2.match(/\[[\s\S]*\]/);var deals2=[];if(m2){try{var a2=JSON.parse(m2[0]);deals2=Array.isArray(a2)?a2.filter(function(t){return t&&t.target&&t.buyer&&t.id;}):[]; }catch(e2){}}console.log('[tx] search_one period='+sp+' found='+deals2.length);return res.status(200).json({success:true,deals:deals2,period:sp,raw_length:t2.length});}catch(e2){return res.status(200).json({success:false,deals:[],error:e2.message});}}
